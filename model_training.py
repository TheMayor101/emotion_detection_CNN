...
class EmotionRecognitionModel:
    def __init__(
        ...
        self.model: Sequential = self.initialize_layers()

        # I chose to use adam as my optimizer, categorical_crossentropy as my
        # loss function and accuracy as the metric to check model performance
        self.model.compile(
            optimizer='adam',
    ...
    def initialize_layers(self) -> Sequential:
        """
    ...
    def visualize_model(self) -> None:
        """
        Graphically visualize the model architecture using visualkeras.
        """
        img: Any = visualkeras.layered_view(
            self.model, to_file='model_visual.png'
        )
        img.show()
    ...
    def train_model(self) -> Any:
        ...
        """
        num_train_imgs: int = sum(
            len(files) for _, _, files in os.walk(self.train_path)
        )
        num_test_imgs: int = sum(
            len(files) for _, _, files in os.walk(self.test_path)
        )

        history: Any = self.model.fit(
            data_augmentation.train_generator,
...
if __name__ == "__main__":
    ...
    emotion_model.save_model('emotion_recognition_model.h5')
