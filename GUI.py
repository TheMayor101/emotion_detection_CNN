...
TEST_FOLDER: str = 'data/test/'
IMG_SIZE: tuple[int, int] = (400, 400)
USER_INITIAL_LIVES: int = 3
MODEL_INITIAL_LIVES: int = 3
...
class EmotionRecognitionApp:
    def __init__(self, root: tk.Tk) -> None:
        ...
        """
        self.user_lives: int = USER_INITIAL_LIVES
        self.model_lives: int = MODEL_INITIAL_LIVES
        ...
        self.user_lives_label: tk.Label
        self.model_lives_label: tk.Label
    ...
    def setup_ui(self) -> None:
        ...
        label: tk.Label = tk.Label(
            self.root,
            text="Please Select the emotion you recognize in the image",
    ...
    def setup_scoreboard(self) -> None:
        ...
        """
        tk.Label(
            self.root, text="The Score", font=(FONT_TYPE, 16, BOLD_TYPE)
        ).pack()

        self.user_score_label = tk.Label(
            self.root, text=f"User: {self.user_score}", font=(FONT_TYPE, 14)
        )
        self.user_score_label.pack()

        self.model_score_label = tk.Label(
            self.root,
            text=f"Model: {self.model_score}",
            font=(FONT_TYPE, 14),
        )
        ...
        self.user_lives_label = tk.Label(
            self.root,
            text=f"User Lives: {self.user_lives}",
            font=(FONT_TYPE, 14),
        )
        ...
        self.model_lives_label = tk.Label(
            self.root,
            text=f"Model Lives: {self.model_lives}",
            font=(FONT_TYPE, 14),
        )
    ...
    def load_random_image(self) -> None:
        """
        Loading a random image from the test folder and presenting it on the screen
        """
        chosen_folder: str = random.choice(os.listdir(TEST_FOLDER))
        folder_path: str = os.path.join(TEST_FOLDER, chosen_folder)
        random_image_path: str = os.path.join(
            folder_path, random.choice(os.listdir(folder_path))
        )

        img: Image.Image = Image.open(random_image_path).resize(IMG_SIZE)
        img_tk: ImageTk.PhotoImage = ImageTk.PhotoImage(img)

        self.img_label.config(image=img_tk)
        self.img_label.image = img_tk  # type: ignore[attr-defined]
        self.img_path = random_image_path

        correct_answer: str = os.path.basename(
    ...
    def predict_emotion(self) -> None:
        ...
        if self.user_lives <= 0 or self.model_lives <= 0:
            messagebox.showinfo(
                "Game Over",
                "The game is over. You can restart the game or exit it.",
            )
            return

        user_guess: int = self.emotion_var.get()
        img_array = model_handeling.preprocess_image(self.img_path)
        predicted_class: int = model_handeling.predict_emotion(img_array)

        result_text: str = (
            f"CNN Model Prediction: {EMOTIONS[predicted_class]}\n"
            f"Your Guess: {EMOTIONS[user_guess]}\n"
            f"Correct Answer: {EMOTIONS[self.correct_answer_index]}\n"
        )
        ...
        result_text += (
            f"Remaining Lives - User: {self.user_lives}, "
            f"Model: {self.model_lives}\n"
        )

        if self.user_lives == 0 or self.model_lives == 0:
            winner: str = (
                "User" if self.user_lives > 0
                else "Model" if self.model_lives > 0
                else "No one"
            )
